...
function imgDownload(set, i){
	...
	download(set[i].uri, __dirname+'/'+'img/'+set[i].id, function(){
		...
		if(i >= 0) {
			setTimeout(function(){imgDownload(set, i-1);},1500);
...
function downImg(){
	console.log('downImg');
	connection.query(
		'SELECT id, title FROM amazon.xunleitai  where clazz<>"cl"; ', function(error, results, fields) {
			if (error) {
			...
			for(var a in results) {
				var row = results[a];
				if(row.title) {
					rows.push({id:row.id, uri:'/j/subject_suggest?q='+encodeURI(row.title)});
				}
			}
