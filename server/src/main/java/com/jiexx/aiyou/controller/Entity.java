...
public class Entity extends DataService {
	...
	@ResponseBody
    public String reg_sellor(
    		@RequestParam(value = "n") String name,
    		@RequestParam(value = "i") String intro,
    		@RequestParam(value = "c") String call,
    		@RequestParam(value = "p") String img,
	...
	@RequestMapping(value="/reg_driver.do", params = {"n", "g", "i", "r", "c", "p", "a"}, method=RequestMethod.GET)
	@ResponseBody
    public String reg_driver(
    		@RequestParam(value = "n") String name,
    		@RequestParam(value = "g") String gender,
    		@RequestParam(value = "i") String intro,
    		@RequestParam(value = "r") String car,
    		@RequestParam(value = "c") String call,
    		@RequestParam(value = "p") String img,
    		@RequestParam(value = "a") String avatar)
	{
		...
		Integer u = DATA.insertDriver(Long.parseLong(call), name, gender, intro, car, Const.CLZ_LOVE.str(), 20, Util.base64ToBytes(avatar), img);
		if( u != null && u == 1 ) {
			String md5 =  DigestUtils.md5DigestAsHex(String.valueOf(System.currentTimeMillis()).getBytes());
			DATA.updateUser(Long.parseLong(call), md5);
			resp.err = Const.SUCCESS.val();
